...
#### Core Features
- **Responsive UI** — Full `flutter_screenutil` integration for all widgets (`.w`, `.h`, `.r`, `.sp`)
...
- **ScreenUtil Support** — All UI components now responsive with automatic scaling
...
- **ScreenUtil Required** — Apps must now wrap with `ScreenUtilInit` for proper scaling
...
- **Responsive Design** — All widgets now scale properly across all device sizes
...
- **ScreenUtil Integration** — Fixed widget scaling on tablets and foldable devices
...
- `CloudMedia.pick(enableEditing: bool)` — Use `enableCropping` instead (scheduled for removal in 0.0.6)
...
dependencies:
  cloud_media: ^0.0.5
  flutter_screenutil: ^5.9.3  # NEW - Required for responsive UI
